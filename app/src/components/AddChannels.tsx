...
export default function AddChannels() {
  ...
  const addedChannels: Channel[] = loading
    ? []
    : storage.addedChannels.twitch.map(username => {
  ...
  function onAdd(username: string): void {
    setStorage({
      addedChannels: {
        ...
        twitch: uniq(storage.addedChannels.twitch.concat(username.toLowerCase())),
      },
    }, true);
  }
  ...
  function onRemove(username: string): void {
    setStorage({
      addedChannels: {
        ...storage.addedChannels,
        twitch: storage.addedChannels.twitch.filter(u => u !== username.toLowerCase()),
      },
    }, true);
  }
